...
public class SwerveDrivetrain extends SubsystemBase {
        ...
        public void periodic() {
                ...
                if (Utils.deadZones(m_gyro.getRate(), 0.05) != 0) { // checks rotation, always is a value bc vibrate ->
                        ...
                        m_isTurning = true;
                } else if (m_isTurning = true && Utils.deadZones(m_gyro.getRate(), 0.05) == 0) { // if deadzone/getRate
                                                                                                 // is 0, so
                                                                                                 // not
                                                                                                 // turning, but
                                                                                                 // m_isTurning is
                                                                                                 ...
                                                                                                 // want to do smth
                        m_isTurning = false;
                        m_pidController.setSetpoint(Math.toRadians(m_gyro.getAngle()) % (Math.PI * 2)
                                        + (Math.PI * 2) % (Math.PI * 2)); // store
                                                                          // heading,
                                                                          // keep
                                                                          // degrees
                                                                          // for
                                                                          // now
                        m_rotSpeed = m_pidController.calculate(Math.toRadians(m_gyro.getAngle()) % (Math.PI * 2)
                                        + (Math.PI * 2) % (Math.PI * 2)); // gets
                                                                          // the
                                                                          // error
                                                                          // to
                                                                          // correct
                                                                          ...
                                                                          // gyro
                                                                          // angle
                } else if (m_xSpeed != 0 || m_ySpeed != 0) { // if moving at all, assume drift, but if rotating, then
                                                             // first
                                                             // conditional just sets isTurning as true, no heading
                                                             // correction
                        m_rotSpeed = m_pidController.calculate(Math.toRadians(m_gyro.getAngle()) % (Math.PI * 2)
                                        + (Math.PI * 2) % (Math.PI * 2)); // if
                                                                          // off,
                                                                          // gives
                                                                          // correction
                                                                          // as
